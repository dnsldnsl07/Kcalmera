...
def add_final_retrain_ops(class_count, final_tensor_name, bottleneck_tensor,
  ...
  with tf.name_scope('train'):
    optimizer = tf.train.GradientDescentOptimizer(my_learning_rate)
...
def run_final_eval(train_session, module_spec, class_count, image_lists,
  ...
  with tf.gfile.GFile("final_test_acc_" + str(test_accuracy * 100), 'w') as f:
...
def main(_):
  global my_learning_rate 
  my_learning_rate = FLAGS.learning_rate
  ...
  logging.info("##### CHECK #####: Next line should log Img_076_0722.jpg")
  logging.info("image_lists['bap']['training'][20]: " + image_lists['bap']['training'][20])
  ...
  with tf.Session(graph=graph) as sess:
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
        ...
        logging.info('%s: Step %d: learning rate = %f',
                     datetime.now(), i, my_learning_rate)
